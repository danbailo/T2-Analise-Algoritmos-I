...
if __name__ == '__main__':
    ...
    print(sem(avg_iterative))
    print(np.std(avg_iterative))


    print('Erro padrão:', sem(time_iterative))
    print('Desvio Padrão:', np.std(time_iterative))
    print('Variância:', np.var(time_iterative))

    # plt.title('Instância/Tempo - Tempo médio para execução de uma instância')
    # plt.xlabel('Tempo/s')
    # plt.ylabel('Resultado')
    # plt.ylim(-0.3,1.5)
    # plt.bar(name_instance, time_iterative, yerr=np.var(time_iterative), color='orange', label='Iterativo = '+str(f'{np.sum(time_iterative):e}')+' seg')
    # plt.errorbar(name_instance, time_iterative, yerr=sem(time_iterative), uplims=True, lolims=True, color='orange', label='Iterativo = '+str(f'{np.sum(time_iterative):e}')+' seg')
    # plt.errorbar(name_instance, time_recursive, yerr=np.var(time_recursive), color='steelblue', label='Recursivo = '+str(f'{np.sum(time_recursive):e}')+' seg')
    # plt.xticks(rotation=90, ha='right', fontsize=7)
    # plt.legend(fontsize=7,loc='upper left')
    # plt.grid()
    # plt.savefig('./img/time_two.pdf')
